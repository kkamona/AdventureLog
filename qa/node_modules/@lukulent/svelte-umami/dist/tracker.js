import { status } from './stores/umami';
 ...
 * @returns
 */
export function trackEventWithProperties(eventName, properties, eventData) {
    if (!browser)
        return Promise.resolve('');
    if ([undefined, 'error', 'removed'].includes(get(status)))
        return Promise.resolve('Umami not found.');
 ...
 */
async function registerUmamiScript(websiteID, srcURL = 'https://eu.umami.is/script.js') {
...
export async function registerUmami(websiteID, srcURL, configuration) {
    // We add the script only once even when the component rendered twice and don't run while SSR.
    if (!browser || window.document.getElementById(SCRIPT_ID))
        return;
    try {
...
function setScriptSettingsProps(scriptElem, config) {
    if (config['data-host-url'])
        scriptElem.setAttribute('data-host-url', config['data-host-url']);
    ...
    if (config['data-domains'])
        scriptElem.setAttribute('data-domains', config['data-domains']);
    if (config['data-exclude-search'])
        scriptElem.setAttribute('data-exclude-search', 'true');
    if (config['data-tag'])
        scriptElem.setAttribute('data-tag', config['data-tag']);
}
/**
...
export function handleEvent(e) {
    ...
    const targetUmamiId = e.target?.getAttribute('data-umami-event');
    ...
    trackEvent(`${targetUmamiId ?? currentTargetUmamiId ?? targetId ?? currentTargetId} ${e.type}`, e);
}
